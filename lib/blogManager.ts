...
export function getPostCountInYear(groupedPosts: GroupedPosts, year: number): number {
  let postCount = 0;
  for (const [_, days] of groupedPosts.get(year.toString()) ?? []) {
    for (const [_, posts] of days) {
